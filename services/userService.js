...
class UserService {
    ...
    async getAll(company_id) {
        try {
            const users = await User.find({company_id: company_id, is_admin: false}).sort({
                created_at: 1,
            });
    ...
    async backToAdmin(req) {
        const defaultCompanyId = "6491b770918d5a3c32ddbf71";
        const defaultName = "CRM";
    ...
    async changePassword(id, rePassword) {
        try {
            ...
            const changedUserPassword = await User.findByIdAndUpdate(id, {
